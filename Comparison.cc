...
OrderMaker::OrderMaker(Schema *schema)
{
    ...
    // not only record the corresponding type of each attribute
    // but also align them from Int to Double then String
...
int CNF::GetSortOrders(OrderMaker &left, OrderMaker &right)
{
    ...
    // loop through all of the disjunctions in the CNF
    // find those that are acceptable for use in a sort ordering
    for (int i = 0; i < numAnds; i++) {

        // if we don't have a disjunction of length one
        // then it can't be acceptable for use with a sort ordering
        // TODO ??? refer to CNF disjunction rules to find out more
        if (orLens[i] != 1) {
            continue;
        }
        ...
        // so all we need to do is add the new comparison info into the relevant structures
        ...
        for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
            ...
            if (myOr->left->left->code == NAME) {
                ...
                if (leftSchema->Find(myOr->left->left->value) != -1) {
                    ...
                    cnf.orList[whichAnd][whichOr].whichAtt1 = leftSchema->Find(
                    ...
                    cnf.orList[whichAnd][whichOr].whichAtt1 = rightSchema->Find(
                            myOr->left->left->value);
                ...
                cerr
                        << "You gave me some strange type for an operand that I do not recognize!!\n";
                exit(1);
            }
            ...
            if (myOr->left->right->code == NAME) {
                ...
                if (leftSchema->Find(myOr->left->right->value) != -1) {
                    ...
                    cnf.orList[whichAnd][whichOr].whichAtt2 = leftSchema->Find(
                    ...
                    cnf.orList[whichAnd][whichOr].whichAtt2 = rightSchema->Find(
                            myOr->left->right->value);
        ...
        for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
            ...
            if (myOr->left->left->code == NAME) {
                ...
                if (mySchema->Find(myOr->left->left->value) != -1) {
                    cnf.orList[whichAnd][whichOr].operand1 = Left;
                    cnf.orList[whichAnd][whichOr].whichAtt1 = mySchema->Find(
                            myOr->left->left->value);
            ...
            if (myOr->left->right->code == NAME) {
                ...
                if (mySchema->Find(myOr->left->right->value) != -1) {
                    // TODO check the logic to see if this modification works
                    //					cnf.orList[whichAnd][whichOr].operand2 = Left;
                    cnf.orList[whichAnd][whichOr].operand2 = Right; // TODO NO This is wrong since Right represents right schema however there is only one schema here
                    cnf.orList[whichAnd][whichOr].whichAtt2 = mySchema->Find(
                            myOr->left->right->value);
